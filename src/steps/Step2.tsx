...
const initialIndividuals: KeyIndividual[] = [
  { id: 'alex', name: 'Alex Carter', detail: 'Director and UBO authorised to sign documents' },
]
...
interface Step2Props { onComplete: () => void }

export function Step2({ onComplete }: Step2Props) {
  const [phase, setPhase] = useState<Step2Phase>('overview')
  const [individuals, setIndividuals] = useState<KeyIndividual[]>(initialIndividuals)
      ...
      <AnimatePresence mode="wait">
        {phase === 'overview' && (
          <motion.div key="overview" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.22, ease: EASE }} style={{ display: 'flex', flexDirection: 'column', gap: 32 }}>
            <UserBubble text="Identity verification completed" />
            ...
            <AiBubble>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12, width: '100%' }}>
                {individuals.map(person => (
                  <div key={person.id} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <div style={{ width: 40, height: 40, borderRadius: 8, border: '1px solid #e6ebeb', backgroundColor: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                    ...
                    <div style={{ minWidth: 0, flex: 1 }}>
                      <p style={{ ...T, color: '#121621', margin: 0 }}>{person.name}</p>
                      <p style={{ ...T, color: '#121621', margin: 0 }}>{person.detail}</p>
                    </div>
                    <button type="button" aria-label="Edit key individual" style={{ background: 'none', border: 'none', padding: 4, cursor: 'pointer', color: '#121621' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
                <button type="button" onClick={onComplete} style={{ minHeight: 44, backgroundColor: '#277777', border: '1px solid #277777', borderRadius: 4, padding: '8px 16px', cursor: 'pointer', fontFamily: 'Inter, sans-serif', fontSize: 14, fontWeight: 500, lineHeight: '18px', color: 'white', boxShadow: 'inset 0px -2px 0px rgba(0,0,0,0.16)' }}>
                  Continue
                </button>
              </div>
            </AiBubble>
        ...
        {phase === 'add' && (
          <motion.div key="add" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.22, ease: EASE }} style={{ display: 'flex', flexDirection: 'column', gap: 32 }}>
            <UserBubble text="Confirmed Alex Carter's details are correct" />
